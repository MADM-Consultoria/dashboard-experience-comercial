...
import { vendaGanhaDiarioPorConsultor } from './_shared/dashboardAgregacoes.js';

/**
 * Expõe a contagem de venda ganha por dia e por consultor, a partir do cache central do
 * dashboard — não consulta o Postgres diretamente. Mesma definição de venda-ganha-periodo.ts,
 * só que agrupado por dia também, pro comparativo mês atual x mês passado do Plano de Ação.
 */
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    const linhas = vendaGanhaDiarioPorConsultor(dados, inicio, fim);
    ...
    console.error('Falha ao ler cache de venda ganha diário por colaborador:', err);
