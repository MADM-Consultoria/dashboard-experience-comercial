...
export interface VendaGanhaDiarioColaboradorLinha {
  ...
  for (let tentativa = 1; tentativa <= tentativas; tentativa++) {
    try {
      ...
      if (resposta.status < 500 || tentativa === tentativas) {
        throw new Error(dados.error ?? 'Não foi possível carregar a venda ganha do colaborador.');
      }
      ultimoErro = new Error(dados.error ?? 'Não foi possível carregar a venda ganha do colaborador.');
    } catch (err) {
      ultimoErro = err;
...
/** Venda ganha por dia de um único colaborador no intervalo — mesma view de
 * fetchVendaGanhaDiarioTodos, filtrado no cliente por nome normalizado (mesmo padrão de
 * fetchProtocoladosDiarioColaborador). Usado no comparativo mês atual x mês passado do card do
 * Plano de Ação, buscado só quando a pessoa pede. */
export async function fetchVendaGanhaDiarioColaborador(token: string, nomeColaborador: string, inicio: string, fim: string): Promise<VendaGanhaDiarioColaboradorLinha[]> {
  const params = new URLSearchParams({ inicio, fim });
  const dados = await fetchComRetry(`/api/venda-ganha-diario-colaborador?${params.toString()}`, token);
 ...
 * precisamos da série diária de vários colaboradores ao mesmo tempo (ritmo diário de Venda
 * Ganha no Plano de Ação). Uma chamada só ao banco em vez de uma por colaborador exibido. */
export async function fetchVendaGanhaDiarioTodos(token: string, inicio: string, fim: string): Promise<Map<string, VendaGanhaDiarioColaboradorLinha[]>> {
  const params = new URLSearchParams({ inicio, fim });
  const dados = await fetchComRetry(`/api/venda-ganha-diario-colaborador?${params.toString()}`, token);

  const porConsultor = new Map<string, VendaGanhaDiarioColaboradorLinha[]>();
