...
/** Colunas fixas pra todo item da lista respeitar a mesma grade — sem isso, badges que
 * quebram linha ou larguras "no chute" por item deixam a lista com cara de bagunçada. */
const GRID_LINHA = 'grid grid-cols-[40px_minmax(0,1fr)_36px_140px_96px_18px] items-center gap-4';
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <KpiCard titulo="Processos represados" valor={formatNumero(perdaTotal)} icon={TrendingDown} accent="danger" />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[13px] text-slate-500 hover:bg-slate-50 transition-colors"
          >
                  ...
                  <button onClick={() => toggle(g.id)} className={clsx(GRID_LINHA, 'w-full px-5 py-4 text-left hover:bg-slate-50 transition-colors')}>
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide" style={{ color: cor, backgroundColor: `${cor}1a` }}>
                          {STATUS_LABEL[g.severidade]}
                        </span>
                        <span className="shrink-0 text-[11px] text-slate-400">{TIPO_LABEL[g.tipo]}</span>
                      </div>
                    ...
                    <div>
                      <p className="text-[11px] text-slate-500 mb-1 truncate">{TIPO_IMPACTO_LABEL[g.tipo]}</p>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-semibold text-slate-700 w-8 text-right">{formatPct(g.impactoPct, 0)}</span>
                      </div>
                    </div>
